...
void muteAc(Ac *ac,int nbMutations)
{
 ...
 int i1; //Le premier indice 
 int i2; //Le deuxièe indice
 int temp; //Pour permuter les deux cases

 for(i = 0; i < nbMutations; i++){

    //Valeur aléatoire récupérée
    i1 = myRandomMinMax(0, ac->nbVilles-1);
    i2 = myRandomMinMax(0, ac->nbVilles-1);
    
    //Retire tant que les deux indices sont les mêmes 
    while (i1 == i2) {
      i2 = myRandomMinMax(0, ac->nbVilles-1);
    }
    ...
    temp = ac->parcours[i1];
    ac->parcours[i1] = ac->parcours[i2];
    ac->parcours[i2] = temp;
    calculCoutAc(ac);
    //tirage aléatoire de deux indices i1 et i2 => i1 != i2
    //Inversion des deux cases du tableau i1 devient i2 et i2 devient i1 
}
}
