                 ...
                 int *nbClones, int *nbNouveaux)
{
 ...
 if (argc==6) {
  ...
  if (printUsage)
  {
   ...
   fprintf(stderr,"\n");
   fprintf(stderr,"Ou              TAILLEPOP => nbIndividus\n");
   fprintf(stderr,"Ou     N : %% de TAILLEPOP => nbClones\n");
   fprintf(stderr,"Ou     D : %% de TAILLEPOP => nbNouveaux\n");
   fprintf(stderr,"Ou          NBGENERATIONS => nbGenerations\n");
   fprintf(stderr,"Ou NBGENERATIONSINJECTION => nbGenerationsInjection\n");
...
#if 1
{
  //Teste d'un seul anticorps pour voir si la mutation fonctionne
  Ac ac;
  genereAc(&ac);
  printAc(&ac);
  muteAc(&ac, 1);
 ...
 while (tour!=nbGenerations)
 {
  ...
  //Si le nouveau meilleur est plus efficace que celui actuel
  //Deviens le nouveau meilleur 
